...
1. Set the base amount and currency, then choose a pricing strategy.
2. Paste a Paddle price id (`pri_...`) and load the current catalog values.
3. Check the preview. Markets Paddle does not bill (for example IDR or AED) are listed as skipped.
4. Apply the update. On production, confirm the warning before the write goes through.
